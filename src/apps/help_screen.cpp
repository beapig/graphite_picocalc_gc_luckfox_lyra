...
const char* const kKeysLines[] = {
    ...
    "Alt+ENTER  decimal result;",
    "  on an empty line, redo the",
    "  last exact one as decimal",
    "#GRAPH",
    "F4 TRC   toggle trace",
    "F5 TBL   value table",
...
const char* const kSyntaxLines[] = {
    ...
    // Glyphs are written as 3-digit octal escapes (\214 = kGlyphSqrt 0x8C,
    // \177 = kGlyphPi 0x7F) so a following digit can't be swallowed into the
    // escape the way it would with \x — no string concatenation needed.
    "#EXACT FORMS",
    "Results with a closed form",
    "show in amber: sqrt(8) as",
    "2\2142, pi*2 as 2\177, 1/3, and",
    "sin(pi/3) as \2143/2. Use >dec",
